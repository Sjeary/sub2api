...
func TestApplyRedeemBalanceAdjustment_UsesAtomicFloor(t *testing.T) {
	...
	mock.ExpectExec(`UPDATE users SET balance = CASE WHEN \$1::numeric >= 0 THEN balance \+ \$1\s+ELSE LEAST\(balance, GREATEST\(balance \+ \$1, 0\)\) END, updated_at = NOW\(\) WHERE id = \$2 AND deleted_at IS NULL`).
